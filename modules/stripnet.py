...
class DWConv(nn.Module):
    """Depthwise 3x3 conv used inside the MLP."""
...
class StripBlock(nn.Module):
    """Strip convolution block with orthogonal depthwise kernels."""
...
class Block(nn.Module):
    ...
    def forward(self, x):
        x = x + self.drop_path(self.layer_scale_1.view(-1, 1, 1) * self.attn(self.norm1(x)))
        x = x + self.drop_path(self.layer_scale_2.view(-1, 1, 1) * self.mlp(self.norm2(x)))
        return x


class OverlapPatchEmbed(nn.Module):
    """Convolutional patch embed that keeps overlap between tokens."""
